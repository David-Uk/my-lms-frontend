...
export function proxy(request: NextRequest) {
  ...
  console.log("[Proxy] Header token exists:", !!headerToken);
  ...
  try {
    ...
    if (!userRole) {
      console.log("[Proxy] No role found in token, redirecting to login");
      const loginUrl = new URL("/login", request.url);
      loginUrl.searchParams.set("redirect", pathname);
      return NextResponse.redirect(loginUrl);
    }
    ...
    if (
      ...
      console.log(
        "[Proxy] Redirecting non-admin from /admin to /tutor/dashboard. Role was:",
        userRole,
      );
    ...
    // Superadmin routes - client-side only for now (proxy cookie issues)
